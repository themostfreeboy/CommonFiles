...
void Zlib_File_Compress(const char* Origin_filename, const char* Zlib_Compress_filename)//文件zlib算法压缩
{
	long filesize=Zlib_MyGetFileSize(Origin_filename);//获取文件大小，调用此函数时此文件必须处于未打开读写状态
...
void Zlib_File_Uncompress(const char* Origin_filename, const char* Zlib_Compress_filename)//文件zlib算法解压缩
{
	long filesize=Zlib_MyGetFileSize(Origin_filename);//获取文件大小，调用此函数时此文件必须处于未打开读写状态
	errno_t err_read, err_write;
	FILE *fp_read = NULL;
...
long Zlib_MyGetFileSize(const char* filename)//获取文件大小，调用此函数时此文件必须处于未打开读写状态
{
	FILE *fp = NULL;
	errno_t err;
